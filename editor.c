...
int main(int argc, char *argv[]){
	...
	printw("ALCHEMIST TEXT EDITOR");
        wattroff(stdscr,A_UNDERLINE);
        move(by/2,bx/3); printw("No File Name provided :("); mvprintw(by/2 + 2,bx/3,"PRESS ENTER TO CREATE A NEW FILE or ANY OTHER KEY TO EXIT :->");
        mg1 = getch();
        if(mg1 != '\n'){
            endwin();
            exit(0);
